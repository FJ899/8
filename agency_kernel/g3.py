...
class Observer:
    ...
    def observe(self, resource: str, *, covered: bool = True, attribution_ambiguous: bool = False) -> Observation:
        ...
        try:
            ...
            prov = c.execute(
                "SELECT mutation_id, admission_id, operation_digest FROM mutation_provenance WHERE resource = ? ORDER BY rowid DESC LIMIT 1",
                (resource,),
            ).fetchone()
            return Observation(
                resource,
                ...
                None if prov is None else prov["mutation_id"],
                None if prov is None else prov["admission_id"],
                None if prov is None else prov["operation_digest"],
                covered,
...
class Kernel(G2Kernel):
    """G3 extension: supported versioned target primitive plus observation/compliance semantics."""
    ...
    def execute_put_if_version_admission(self, admission_id: str, *, crash_point: Optional[str] = None) -> PutResult:
        ...
        try:
            ...
            if crash_point == "before_mutation":
                t.execute("ROLLBACK")
                return PutResult(False, "crash_before_mutation", op.resource, current_version, current_version, operation_digest=digest)
...
class DishonestPrimitive:
    """Negative-control mutant, deliberately outside supported primitive set."""
